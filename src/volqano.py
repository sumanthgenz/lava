...
from aai.experimental.sgurram.lava.src.utils import get_src_conditional_mask, position_embed, position_embed_3d, pad_batch, is_nan, nan_filter
...
class Volqano(nn.Module):
    def __init__(self, 
        ...
        self.codebook = nn.Parameter(torch.randn(self.seqlen, self.num_embeddings, self.model_dimension))
    ...
    def loss(self, a, v, t, t_mask):
        ...
        codes = torch.nn.functional.normalize(self.codebook, p=2, dim=-1).permute(0,2,1)
        ...
        av_loss = 0.5 * (ce_loss(a_onehot, v_label) + ce_loss(v_onehot, a_label))
        vt_loss = 0.5 * (ce_loss(v_onehot, t_label) + ce_loss(t_onehot, v_label))
        ...
        a_prob = (a_onehot.view(self.batch_size, self.seqlen, -1)).transpose(0,1).sum(dim=1)
        v_prob = (v_onehot.view(self.batch_size, self.seqlen, -1)).transpose(0,1).sum(dim=1)
        t_prob = (t_onehot.view(self.batch_size, self.seqlen, -1)).transpose(0,1).sum(dim=1)
