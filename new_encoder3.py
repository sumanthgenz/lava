...
class LAVA(nn.Module):
    def __init__(self, 
        ...
        self.a_at_proj = nn.Linear(self.model_dimension, self.model_dimension)
        self.a_av_proj = nn.Linear(self.model_dimension, self.model_dimension)
        self.a_avt_proj = nn.Linear(self.model_dimension, self.model_dimension)

        self.t_at_proj = nn.Linear(self.model_dimension, self.model_dimension)
        self.t_vt_proj = nn.Linear(self.model_dimension, self.model_dimension)
        self.t_avt_proj = nn.Linear(self.model_dimension, self.model_dimension)


        self.v_av_proj = nn.Sequential(
                    nn.Linear(self.model_dimension, self.model_dimension),
                    nn.BatchNorm1d(self.model_dimension),
                    nn.GELU(),
                    nn.Linear(self.model_dimension, self.model_dimension),
        )
        self.v_vt_proj = nn.Sequential(
                    nn.Linear(self.model_dimension, self.model_dimension),
                    nn.BatchNorm1d(self.model_dimension),
                    nn.GELU(),
                    nn.Linear(self.model_dimension, self.model_dimension),
        )
        self.v_avt_proj = nn.Sequential(
                    nn.Linear(self.model_dimension, self.model_dimension),
                    nn.BatchNorm1d(self.model_dimension),
                    nn.GELU(),
                    nn.Linear(self.model_dimension, self.model_dimension),
    ...
    def loss(self, a, v, t, t_mask):
        ...
        a_avt = self.norm(self.a_avt_proj(a))
        v_avt = self.norm(self.v_avt_proj(v))
        t_avt = self.norm(self.t_avt_proj(t))

        a_av = self.norm(self.a_av_proj(a))
        v_av = self.norm(self.v_av_proj(v))

        a_at = self.norm(self.a_at_proj(a))
        t_at = self.norm(self.t_at_proj(t))

        v_vt = self.norm(self.v_vt_proj(v))
        t_vt = self.norm(self.t_vt_proj(t))

        # approximate centroid vector
        centroid = (a_avt + v_avt + t_avt)/3
        centroid = self.norm(centroid)

        avt_loss = nce_loss(a_avt, centroid, temp=self.temperature)
        avt_loss += nce_loss(v_avt, centroid, temp=self.temperature)
        avt_loss += nce_loss(t_avt, centroid, temp=self.temperature)

        av_loss = nce_loss(a_av, v_av, temp=self.temperature)
        at_loss =  nce_loss(a_at, t_at, temp=self.temperature)
        vt_loss = nce_loss(v_vt, t_vt, temp=self.temperature)

        loss = avt_loss + av_loss + at_loss + vt_loss
        ...
        a_cos_sim = torch.triu(a_av @ a_at.T, diagonal=1).mean()
        v_cos_sim = torch.triu(v_av @ v_vt.T, diagonal=1).mean()
        t_cos_sim = torch.triu(t_at @ t_vt.T, diagonal=1).mean()
