...
from aai.experimental.sgurram.lava.downstream.hmdb51.finetune.data import HMDB51Dataset
...
class HMDB51FineTune(ptl.LightningModule):
    def __init__(
        self, 
        input_channels=128,
        linear_dim=128, 
        ...
        super(HMDB51FineTune, self).__init__()
    ...
    def validation_epoch_end(self, outputs):
        get_confusion_matrix(self.confusion_X, self.confusion_Y, self.classes, 'hmdb51_confusion_matrix')
        get_confusion_dictionary(self.confusion_X, self.confusion_Y, self.classes, 'hmdb51_confusion_dictionary')
...
FLAGS = flags.FLAGS
def main(*args):
    dataset = HMDB51Dataset(FLAGS.train_split_path, loader='npy', split=1, prefix='train')
    dataloader = torch.utils.data.DataLoader(dataset, batch_size=32, num_workers=8, shuffle=True)

    v_dataset = HMDB51Dataset(FLAGS.val_split_path, loader='npy', split=1, prefix='test')
