...
class HMDB51DownstreamProbe(ptl.LightningModule):
    def __init__(
        self, 
        input_channels=128,
        linear_dim=128, 
        ...
        super(HMDB51DownstreamProbe, self).__init__()
...
FLAGS = flags.FLAGS
def main(*args):
    dataset = HMDB51Dataset(FLAGS.train_split_path, loader='npy')
    dataloader = torch.utils.data.DataLoader(dataset, batch_size=128, num_workers=8, shuffle=True)

    v_dataset = HMDB51Dataset(FLAGS.val_split_path, loader='npy')
    ...
    probe = HMDB51DownstreamProbe(2*1024, 512)
